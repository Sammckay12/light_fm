...
full_users = virtual_users_data
# full_users = users_data
# full_ratings = ratings_data+virtual_ratings_data
full_ratings = virtual_ratings_data
...
# interests = set([item for sublist in [x['interests'] for x in virtualUsers] for item in sublist])
...
(x['_id'] for x in locations_data))


interactions = dataset.build_interactions(((x['userId'], x['place'], x['weight']) for x in virtual_ratings_data))
